...
|---|---|---|
| Ablation study (`evals/ablations.yaml`, `evals/run_suite.py`) | Yes — 5 variants, verified via `--dry-run` and structural graph-node inspection, zero live cost | Run small, not at the original scale — 5 variants × 5 tasks × 1 trial (25 simulations) against `gpt-4.1-mini`, $0.58, after `gpt-4.1-nano` ($0.12) proved too weak to pass the DB check on any variant. Original plan was 5 variants × 40 tasks × 4 trials (~800 simulations, estimated $40-120). See [`RESULTS.md`'s "Ablation study"](RESULTS.md#ablation-study) for the full provider journey and results, honestly caveated as n=5/variant, not statistically significant |
...
| Groq | `groq/openai/gpt-oss-120b` | **The 2026-08-18 signup bug is genuinely fixed on a re-tried account (verified live 2026-08-24) — ruled out anyway on a real bounded-task test.** `llama-3.3-70b-versatile` (litellm's static pricing entry) is stale; the account's real live catalog (`GET /v1/models`) is `gpt-oss-120b`/`gpt-oss-20b`/`qwen/qwen3.6-27b` and a few others. `groq/openai/gpt-oss-120b` passed both a plain-text check and a real tool-schema call cleanly (correct, well-formed tool call) — then failed a real single-task debug run on `429 rate_limit_exceeded`: the free tier's real limit is **8,000 tokens/minute**, and a single retail-policy-context call alone requests ~4,000 tokens, so two or three real turns exhausts the entire budget. Failed permanently after 4 retries, ungraded. Confirmed via the console's own Limits page — same cap on every real chat model on the account, not one model's quirk. | `GROQ_API_KEY` |
...
| Google AI Studio (Gemini) | `gemini/gemini-3.6-flash` | **A passing tool-schema check is not the same as being usable — ruled out again, differently (verified live 2026-08-24).** The original `gemini-2.5-flash` (commit 5 era, 20 req/day cap) is now a 404, "no longer available to new users." The current `gemini-3.6-flash` passed a plain-text check AND a real tool-schema call cleanly — genuinely promising at that point — but a real bounded single-task debug run immediately hit `429 RESOURCE_EXHAUSTED`: the actual free-tier quota is **5 requests per minute per project per model**, nowhere near enough for one multi-turn conversation (agent + user + guardrail calls exceed 5 before the first task finishes). Failed permanently after tau2's built-in retries, ungraded. | `GEMINI_API_KEY` |
| Cohere | `cohere/command-r-plus-08-2024` | **A genuine litellm compatibility bug, not a billing/signup issue (verified live 2026-08-19).** Free trial is real and generous (1,000 calls/month, 20/min, no card, doesn't expire) and a plain-text connectivity check succeeds — but the moment tau2's retail domain sends its actual tool schemas, litellm's Cohere v2 adapter includes a `name` field the Cohere API rejects outright ("unknown field: parameter 'name' is not a valid field"), failing on the very first call before any conversation happens. (The plain-text check that "passed" first didn't exercise tool-calling at all, which is exactly where this broke — a real lesson for verifying any future candidate: test with the actual tools involved, not just a bare completion.) Zero cost incurred; the failure is before grading, not during. | `COHERE_API_KEY` |
| Ollama + `qwen3:8b` (fully local) | `ollama/qwen3:8b` | **Tried specifically because Qwen models are widely reported as strong at structured tool-calling — worse than `llama3.1` on both axes that matter, not better (live-verified 2026-08-21).** A bounded real retail-task debug run: zero of the required tool calls were ever issued — the model narrated its intent in prose ("I'll use the 'list_all_product_types' function...") instead of a real tool call, and its second turn leaked a raw OpenAI-style streaming delta chunk as literal message content. Separately: 613.72s wall-clock for that single incomplete 2-turn exchange (`qwen3:8b`'s "thinking" mode), which alone would make a full ablation impractically slow even if the output had been usable. | none (local) |
| Ollama + `qwen2.5:7b` (fully local) | `ollama/qwen2.5:7b` | **A second, deliberately different Qwen lineage (no "thinking" mode) — failed differently and more seriously (live-verified 2026-08-21).** Instead of a real tool call, emitted a hallucinated custom JSON "action plan" as literal message text, then on the very next turn fabricated a completed customer return ("The cleaner, headphone, and smart watch... have been marked for return") without ever calling a real tool — a genuine ungrounded-claim hallucination, not just a formatting miss. Then hit the wall-clock cap and looped calling `transfer_to_human_agents` four times in a row (the same escalation-loop bug later fixed — see `RESULTS.md`). Two different Qwen lineages now ruled out for two different, real reasons; a third (larger) Qwen variant wasn't pursued blind, since nothing suggested the failure was size-related rather than a deeper harness/chat-template mismatch. | none (local) |
| Alibaba Cloud Model Studio (Qwen/DashScope) | `dashscope/qwen-plus`, `dashscope/qwen3-max`, etc. | **Ruled out on a jurisdiction block, not a technical or billing issue (2026-08-21).** litellm has broad, current `dashscope/*` coverage and Qwen models are well regarded for tool-calling specifically — but Alibaba Cloud services are banned in India, where this account is based, so no account was ever possible. Never reached a technical evaluation. | n/a |
| `gpt4free` (xtekky/gpt4free on GitHub) | n/a | **Rejected on integration grounds after re-examining it live, not ToS objections alone (2026-08-19).** It's a real, pip-installable package, but not a litellm provider at all (confirmed by grepping the vendored litellm package: zero matches) — would need real new adapter code, not a config swap. Many of its providers rely on Chrome/Chromium browser automation and scraped session cookies rather than stable API keys, a poor fit for an unattended multi-simulation sweep, and it makes no tool-calling reliability claims whatsoever — the exact axis that already broke Ollama's models. Not worth the engineering time even at zero dollars. | n/a |
...
unexpected billing wall, a tighter cap, a tool-schema incompatibility, a fabricated
tool call, or — for the one that ran cleanly — quality too low to trust for anything
where response quality itself is what's being measured).

**A passing connectivity check, even with real tool schemas, doesn't prove real
workload viability.** Gemini's `gemini-3.6-flash` and Groq's `gpt-oss-120b` both
cleared a plain-text check *and* a real tool-schema call (correct, well-formed tool
calls both times) — then failed the moment an actual bounded task ran, on rate limits
neither check could surface (Gemini: 5 requests/minute; Groq: 8,000 tokens/minute,
exhausted by 2-3 real turns since a single retail-policy-context call alone needs
~4,000 tokens). The only check that actually settles viability is a real bounded task
run (`max_steps=12` against one real domain task) — a single minimal
`litellm.completion(..., max_tokens=5)` call tells you the key and model name work,
nothing about whether the free tier can sustain a real conversation's request/token
volume. Verify against your own account directly, with a real bounded task, before
planning a run around any of them — and when the study needs to measure whether the
*production* model resists something (like the adversarial suite), a small amount of
real spend against the real model may be unavoidable and is usually cheap: the Haiku
adversarial run above cost $0.24 for 28 cases.
